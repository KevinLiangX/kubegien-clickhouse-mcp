...
def run_huarun_log_query(query: str):
    ...
    - **Business Logs**: Stored in `logs.log_file_all` (Distributed table).
      - Always query `logs.log_file_all` for logs, NOT `logs.log_file_local`.

    ## Filtering Guide (Field Mapping)
    The following columns are the PRIMARY filters. Combine them using `AND` logic when multiple criteria are mentioned.

    | User Intent / Concept | Column Name | Type | Description |
    |-----------------------|-------------|------|-------------|
    | Time / Date | `createdtime` | DateTime64 | Event timestamp. Always include a time range (e.g., last 1h). |
    | Cluster | `cluster_name`| String | The Kubernetes cluster name. |
    | Node / Machine | `node_name` | String | The node where the pod is running. |
    | App / Service | `app_name` | String | Name of the application or microservice. |
    | Namespace / Env | `namespace` | String | Kubernetes namespace (e.g., prod, dev). |
    | Workload Type | `workload_type`| String | e.g., Deployment, StatefulSet. |
    | Workload Name | `workload_name`| String | The specific name of the workload controller. |
    | Log File | `log_name` | String | The specific log filename. |
    | Log Content / Keyword | `message` | String | The actual log text. Use `ILIKE` for case-insensitive search. |

    **Note**: `id`, `dbcreatedtime`, `expiretime`, `file_offset` are internal system fields and should rarely be used for filtering unless explicitly requested.
...
if get_config().enabled:
    ...
    # Register the business prompt explicitly ONLY if enabled
    if get_config().huarun_context:
        # Register the specialized tool for Huarun
